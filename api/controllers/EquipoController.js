...
module.exports = {
	...
	getByCurso: function (req,res) {
		...
		if (!curso_id) {
			return res.badRequest({code:1, msg: 'Se debe ingresar el código del curso'});
		  }
		
		  semestre_id = req.param('semestre_id');
		if (!semestre_id) {
			return res.badRequest({code:1, msg: 'Se debe ingresar el código del semestre'});
		}
			...
			include: [
				{
					...
					include: [
					 ...
					 where: {
						materiaCodigo: curso_id
					 }
				}
			]
		}).then(resEquipos =>{
			if(resEquipos.length<=0){
				return res.badRequest({code:1, msg: 'No hay grupos para el curso y semestre indicados '});
			}
			return res.ok(resEquipos);
		})	
		.catch(err => {
			return res.serverError(err);
		});
